...
class HypothesisTesting():
    """
        Main Class for hypothesis testing
    ...
    def find40tile(self):
        ...
        min_income = self.sc.broadcast(dict(self.complete_data.map(lambda r: (r[0], r[1][2]))\
            .reduceByKey(lambda a,b: min(a,b))\
            .collect()))

        # contains maximum income of the population
        max_income = self.sc.broadcast(dict(self.complete_data.map(lambda r: (r[0], r[1][2]))\
        ...
        while checkIfUnfinished():
            ...
            lower_population = dict(self.complete_data.filter(lambda row: row[0] not in filter_range and row[1][2]<=mid_income[row[0]]) \
            ...
            for year in lower_population:
                if lower_population[year]<=int(0.4 * self.total_population.value[year]):
            ...
            min_income = self.sc.broadcast(temp_min_inc)
            max_income = self.sc.broadcast(temp_max_inc)
            print("===============================================================")
        print("income_per_capita_upper_margin: ", income_per_capita_upper_margin)
        ...
        self.income_per_capita_upper_margin = self.sc.broadcast(income_per_capita_upper_margin)


    def __init__(self):
        self.sc = SparkContext("yarn", "HypothesisTesting")
        rdd = self.sc.textFile('hdfs:/data/')
        ...
        geoids_to_exclude = self.sc.broadcast(set(unprocessed_data.map(lambda r: (r[1][0], r[0]))\
            .groupByKey()\
            .filter(lambda r: len(r[1])!=8)\
            .map(lambda r: r[0])\
            .collect()))

        self.complete_data = unprocessed_data.filter(lambda r: r[1][0] not in geoids_to_exclude.value)
        self.all_years = self.sc.broadcast(sorted(self.complete_data.map(lambda r:r[0]).distinct().collect()))

        # contains total population 
        self.total_population = self.sc.broadcast(dict(self.complete_data.map(lambda row: (row[0], row[1][1]))\
            .reduceByKey(lambda a,b:a+b)\
            .collect()))
    ...
    def growth_rate_calc(self, row, ay):
    ...
    def second_hypothesis_bloat(self, row, ay):
        retList = []
        for i in range(len(ay)-1):
    ...
    def calc_tvalues_th_hyp(self, row):
        sum_val = row[1][0]
        sum_sqval = row[1][1]
        cnt = row[1][2]
        moment1 = sum_val/cnt 
    ...
    def first_hypothesis(self):
        income_per_capita_upper_margin = self.income_per_capita_upper_margin
        poor_pop = self.complete_data.filter(lambda row: row[1][2]<=income_per_capita_upper_margin.value[row[0]]) \
            .map(lambda row: (row[0], (row[1][1], row[1][1]*row[1][2]))) \
            .reduceByKey(lambda a,b: (a[0]+b[0], a[1]+b[1])) \
            .map(lambda row: (row[0], row[1][1]/row[1][0])) \
            .collect()
        poor_pop = self.sc.broadcast(poor_pop)
        ...
        total_pop = self.complete_data.map(lambda row: (row[0], (row[1][1], row[1][1]*row[1][2]))) \
            .reduceByKey(lambda a,b: (a[0]+b[0], a[1]+b[1])) \
            .map(lambda row: (row[0], row[1][1]/row[1][0])) \
            .collect()
        total_pop = self.sc.broadcast(total_pop)
    ...
    def second_hypothesis(self):
        income_per_capita_upper_margin = self.income_per_capita_upper_margin
        first_year_poor_pop = self.sc.broadcast(set(\
            self.complete_data.filter(lambda r: r[0]=='20062010' and r[1][2]<=income_per_capita_upper_margin.value['20062010'])\
                .map(lambda row:row[1][0])\
                .collect()))

        ay = self.all_years.value.copy()
        first_yr_ = first_year_poor_pop.value

        # y-o-y growth rate (('gr_STARTYEAR', 'GEOID'), GROWTH_RATE)
        second_hypothesis_domain = self.complete_data.filter(lambda r: r[1][0] in first_yr_) \
            .map(lambda r: (r[1][0], (r[0], r[1][2]))) \
            .groupByKey() \
            .flatMap(lambda r: self.growth_rate_calc(r, ay)) \
            .flatMap(lambda r: self.second_hypothesis_bloat(r, ay)) \
            .groupByKey() \
            .map(lambda r: self.create_diff(r))

        second_hypothesis_results = second_hypothesis_domain.reduceByKey(lambda a,b: (a[0]+b[0], a[1]+b[1], a[2]+b[2])) \
            .map(lambda r: self.calc_tvalues(r))\
            .collect()
        ...
        dof = len(self.first_yr_) - 1
        tvalue_005 = 1.6449
        return dof, second_hypothesis_results

    def third_hypothesis(self):
        all_years - self.all_years
        income_per_capita_upper_margin = self.income_per_capita_upper_margin
        # (do_date, (geo_id, total_pop, income_per_capita))
        pop_gr_rt = self.complete_data.map(lambda r: (r[1][0], (r[0], r[1][2]))) \
            .groupByKey() \
            .flatMap(lambda r: self.growth_rate_calc(r, all_years))

        h_not = self.sc.broadcast(dict(pop_gr_rt.map(lambda r: (r[0][0], (r[1], 1)))\
            .reduceByKey(lambda a,b: (a[0]+b[0], a[1]+b[1]))\
        ...
        third_hyp_distribution = self.complete_data.filter(lambda r: r[1][2]<=income_per_capita_upper_margin.value[r[0]]) \
            .map(lambda r: ((r[0], r[1][0]), r[1][2])) \
            .join(self.complete_data.map(lambda r: ((str(int(r[0][:4])-1)+str(int(r[0][4:])-1), r[1][0]),r[1][2]))) \
            .map(lambda r: (r[0][0], (r[1][1] - r[1][0])/r[1][0]))

        t_values = third_hyp_distribution.map(lambda r: (r[0], (r[1], r[1]**2, 1)))\
            .reduceByKey(lambda a,b: (a[0]+b[0], a[1]+b[1], a[2]+b[2]))\
            .map(self.calc_tvalues_th_hyp)
...
if __name__ == "__main__":
    ...
    print("First_Hypothesis: ", hypothesisTesting.first_hypothesis())
    t3 = time.time()
    print("First hypothesis time: ", t3-t2)
    print("Second Hypothesis: ", hypothesisTesting.second_hypothesis())
    t4 = time.time()
    print("Second hypothesis time: ", t4-t3)
    print("third_hypothesis: ", hypothesisTesting.third_hypothesis())
    t5 = time.time()
